  ...
  return new Promise(async (resolve, reject) => {
    ...
    fetch(endpoint, {
      ...
      .then(async (data) => {
        ...
        if (!data.invites) return resolve();
        if (data.msgType && data.msgType === "error") {
        ...
        await localforage.setItem("latestInvites", data.invites);

        return resolve(data.invites);
      });

    setTimeout(() => {
      if (!syncSuccessful) {
        controller.abort(`"Timeout reached (${timeout / 1000} seconds)"`);
        return reject(new Error("syncLatestInvites timed out"));
      }
    }, timeout);
  });
}
